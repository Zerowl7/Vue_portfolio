...
<template>
  ...
  <nav class="bg-light-primary
   border-gray-200 px-2 sm:px-4 py-2.5 rounded
    dark:bg-dark-primary fixed z-20 w-full"
   :class="{
    ...
    <div class="container flex flex-wrap justify-between items-center mx-auto ">
      <a href="https://flowbite.com/" class="flex items-center">
        <img src="https://icon-library.com/images/leaf-icon-png/leaf-icon-png-24.jpg" class="mr-3 h-6 sm:h-9"
          alt="Flowbite Logo">
      ...
      <div class="w-full md:block md:w-auto" :class="{hidden: showMobileMenu}" id="navbar-default">
        <ul class="flex flex-col p-4 mt-4 rounded-lg border
       border-white dark:border-dark-navy-100 
       md:flex-row md:space-x-8 md:mt-0  md:font-medium md:border-0">
          <li v-for="(navigation, index) in navigations" :key="index">
            <a :href="navigation.href" class="block py-2 pr-4 pl-3
            hover:text-light-tail-100
             
             text-black
             rounded 
            dark:text-white" aria-current="page">{{ navigation.name }}</a>
          </li>
